...
export default function AccessControl() {
  ...
  return (
    <div className="animate-view p-6 space-y-8 font-['Cairo'] relative z-10">
      ...
      <div className={`p-8 rounded-[2.5rem] shadow-2xl border-t-4 border-t-gold backdrop-blur-xl ${isDark ? 'bg-black/40 border-white/5' : 'bg-white border-zinc-200 shadow-zinc-200'}`}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-3xl bg-gold/10 flex items-center justify-center text-gold border border-gold/20 shadow-lg animate-pulse">📡</div>
            <div>
              <h3 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-zinc-900'}`}>{isRTL ? "مركز التحقق الميداني" : "Field Access Command"}</h3>
              <p className="text-[10px] font-black text-gold uppercase tracking-[0.4em]">Al-Duwaihi SOC Operations</p>
            </div>
          </div>
          ...
          <select
            ...
            className={`p-5 px-10 rounded-2xl border-2 font-black text-xs outline-none transition-all cursor-pointer ${isDark ? 'bg-black border-white/10 text-white focus:border-gold' : 'bg-zinc-50 border-zinc-200 focus:border-gold text-zinc-900 shadow-inner'}`}
          >
      ...
      <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className={`p-12 rounded-[3.5rem] border-2 shadow-2xl ${isDark ? 'bg-black/40 border-white/5' : 'bg-white border-zinc-100'}`}>
            <form onSubmit={handleVerify} className="relative mb-12">
              <input
                ...
                className={`w-full p-8 rounded-[2.5rem] border-4 text-4xl font-black text-center outline-none transition-all ${isDark ? 'bg-black/60 border-white/5 text-white focus:border-gold' : 'bg-zinc-100 border-zinc-200 focus:border-gold text-zinc-900 shadow-inner'}`}
              />
              <button type="submit" className="absolute top-5 bottom-5 inset-x-6 w-fit px-10 mr-auto bg-gold text-black rounded-2xl font-black text-sm hover:brightness-110 transition-all active:scale-95 shadow-xl shadow-gold/20">
        ...
        <div className="lg:col-span-5 h-full">
          <div className={`p-10 rounded-[3.5rem] border text-center h-full flex flex-col justify-center shadow-2xl relative overflow-hidden ${isDark ? 'bg-black/40 border-white/5' : 'bg-white border-zinc-100 shadow-zinc-200'}`}>
            <div className="absolute top-0 left-0 w-full h-1.5 bg-gold opacity-20"></div>
            <p className="text-[11px] font-black text-zinc-500 uppercase tracking-[0.5em] mb-6">{isRTL ? "المتواجدون حالياً" : "Current Occupancy"}</p>
            <h2 className="text-[10rem] font-black text-gold tracking-tighter tabular-nums leading-none">{onSiteCount}</h2>
