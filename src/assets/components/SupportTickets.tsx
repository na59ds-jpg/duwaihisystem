...
export function SupportTickets() {
  ...
  // Confirm rejection and save to DB
  const confirmReject = async () => {
    if (!rejectReason.trim() || !targetRequestId) return;

    try {
      // تحديث الحالة وتخزين السبب لكي يظهر للموظف في بوابته
      await updateDoc(doc(db, "employee_requests", targetRequestId), {
        status: "مرفوض",
        rejectionReason: rejectReason, // تخزين السبب هنا
    ...
    <div className="p-6 animate-view font-['Cairo'] relative z-10" dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div key={r.id} className={`p-8 rounded-[3rem] border-2 shadow-2xl transition-all hover:border-[#C4B687]/50 ${isDark ? 'bg-black/40 border-white/5 shadow-black' : 'bg-white border-zinc-100'}`}>
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 space-y-6">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span className={`px-4 py-1 rounded-full font-black text-[9px] uppercase bg-[#C4B687] text-black`}>
                      {r.type}
                    </span>
              ...
              <div className="flex flex-row lg:flex-col gap-3 justify-center border-l border-white/5 px-6">
                <button onClick={() => handleApprove(r)} className="w-full lg:w-44 bg-success text-white p-5 rounded-2xl font-black text-xs shadow-xl hover:brightness-110 active:scale-95 transition-all uppercase">إصدار واعتماد ✅</button>
                <button onClick={() => initiateReject(r.id)} className="text-error font-black text-[9px] hover:underline uppercase tracking-tighter transition-all">رفض الطلب وإعادته ✕</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {rejectModalOpen && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`w-full max-w-lg p-8 rounded-[2rem] border-2 shadow-2xl animate-in zoom-in duration-200 ${isDark ? 'bg-zinc-900 border-white/10' : 'bg-white border-zinc-200'}`}>
            <h3 className={`text-xl font-black mb-6 ${isDark ? 'text-white' : 'text-zinc-900'}`}>{isRTL ? "سبب الرفض (إلزامي)" : "Rejection Reason (Required)"}</h3>
            <textarea
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              className={`w-full h-32 p-4 rounded-xl border-2 outline-none font-bold text-sm resize-none mb-6 ${isDark ? 'bg-black border-white/10 text-white focus:border-error' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-error'}`}
              placeholder={isRTL ? "اكتب سبب الرفض هنا ليظهر للموظف..." : "Type reason here..."}
            />
            <div className="flex gap-4">
              <button onClick={confirmReject} className="flex-1 py-4 bg-error text-white rounded-xl font-black shadow-lg hover:brightness-110 active:scale-95 transition-all">{isRTL ? "تأكيد الرفض ✕" : "Confirm Reject"}</button>
              <button onClick={() => setRejectModalOpen(false)} className={`flex-1 py-4 rounded-xl font-black transition-all ${isDark ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>{isRTL ? "إلغاء" : "Cancel"}</button>
            </div>
          </div>
        </div>
      )}
